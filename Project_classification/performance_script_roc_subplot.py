...
logging.basicConfig(filename = 'Model_perfomance2.log', level = logging.INFO, format = '%(asctime)s %(filename)s %(levelname)s %(message)s')
cwd = os.getcwd()
...
for key in weights_dict_2:
    ...
    plt.title(f'DenseNet121_{i}_ROC')
    plt.plot(fpr, tpr, linestyle = '-', label = f"DenseNet121_{i} auc({auc_score:.2f})")
    plt.plot([0 , 1], [0, 1], linestyle = '--')
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive rate')
    plt.legend(loc = 'lower right')
    plt.savefig(f"Dense121_roc{i}.png", dpi = 500)
    plt.close()
    i += 1
